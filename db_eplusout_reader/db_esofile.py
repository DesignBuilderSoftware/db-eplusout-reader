"""Classes representing processed EnergyPlus .eso file output data."""
...
class DBEsoFile:
    ...
    @classmethod
    def from_raw_outputs(cls, raw_outputs, year):
        """Create a DBEsoFile from a single environment's RawOutputData."""
    ...
    def from_path(cls, file_path, year=None):
        """Create a DBEsoFile from a path; raises CollectionRequired if multi-environment."""
        all_raw_outputs = process_eso_file(file_path)
        if len(all_raw_outputs) == 1:
            return cls.from_raw_outputs(all_raw_outputs[0], year)
    ...
    def frequencies(self):
        """List available output frequencies, ordered from most to least granular."""
...
class DBEsoFileCollection:
    ...
    @classmethod
    def from_path(cls, file_path, year=None):
        """Create a DBEsoFileCollection holding one DBEsoFile per environment in the file."""
        all_raw_outputs = process_eso_file(file_path)
        db_eso_files = []
        for raw_outputs in all_raw_outputs:
            db_eso_file = DBEsoFile.from_raw_outputs(raw_outputs, year)
            db_eso_files.append(db_eso_file)
        return cls(db_eso_files)

    @property
    def environment_names(self):
        """List environment names of the held DBEsoFile objects."""
    ...
    def append(self, item):
        """Append a DBEsoFile to the collection."""
    ...
    def extend(self, items):
        """Extend the collection with an iterable of DBEsoFile objects."""
        self._db_eso_files.extend(items)

    def insert(self, index, item):
        """Insert a DBEsoFile at the given index."""
        self._db_eso_files.insert(index, item)

    def pop(self, index):
        """Remove and return the DBEsoFile at the given index."""
        return self._db_eso_files.pop(index)

    def remove(self, item):
        """Remove the first occurrence of the given DBEsoFile from the collection."""
